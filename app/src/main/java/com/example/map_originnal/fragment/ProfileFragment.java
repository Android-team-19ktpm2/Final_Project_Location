...
public class ProfileFragment extends Fragment {
    ...
    Uri mUri;
    ImageView avatar;
    TextView tv_email, tv_dob, tv_full_name,tv_phone, tv_ocp , profile_tv_favourite;
    ...
    private void widgetInit(LinearLayout layout_user)
    {
        ...
        profile_tv_favourite = layout_user.findViewById(R.id.profile_tv_favourite);
    ...
    private void AuthorizedCheck()
    {
        ...
        if (firebaseUser != null){
            ...
            FavoriteRef = FirebaseDatabase.getInstance().getReference("Favorites").child(FirebaseAuth.getInstance().getCurrentUser().getUid());            storageReference = FirebaseStorage.getInstance().getReference(firebaseUser.getUid()+".jpg");
    ...
    private void btnInit() {
        ...
        profile_tv_favourite.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                // Show current User 'favorite
                FavoriteRef.get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                    @SuppressLint("NewApi")
                    @Override
                    public void onComplete(@NonNull Task<DataSnapshot> task) {
                        Iterable<DataSnapshot> places_favorite = task.getResult().getChildren();
                        places_favorite.forEach((element)->{
                            try {
                                Address = xuLyLattoAddress(element);
    ...
    String xuLyLattoAddress(DataSnapshot element) throws IOException {
        Object lat = null;
        Object log = null;
        Iterator<DataSnapshot> temp = element.getChildren().iterator();
        DataSnapshot t = temp.next();
        lat =  t.getValue();
        ...
        System.out.println("0"+lat);
        System.out.println("1"+log);
        Geocoder geocoder = new Geocoder(main);
        List<Address> a = geocoder.getFromLocation(Double.valueOf(lat.toString()),Double.valueOf(log.toString()),1);


        return a.get(0).getAddressLine(0);
    }
